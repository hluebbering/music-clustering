...
Through machine learning and collaborative filtering-related approaches, we can break down and analyze a playlist to gain insights into listening preferences and habits while also recommending similar tastes in music. For example, if a playlist contains several songs from a particular artist, the algorithm may recommend similar artists based on the listening habits of other users with similar music tastes. 


### Next Steps 

Future steps to this project include using Natural Language Processing to track music. NLP searches for mentions of a particular artist or song to analyze current trends and pop culture. New technologies can identify top keywords and terms related to a song and then make recommendations based on similar terms through NLP. Essentially NLP can categorize songs into groups and give each track descriptive adjectives to further classify individual songs. 
