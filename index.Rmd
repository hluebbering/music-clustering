...
df = pd.read_csv('scripts/playlists.csv', encoding_errors='ignore', index_col=0, header=0)

feature_cols = ['acousticness', 'danceability', 'duration_ms', 'energy', 'instrumentalness', 'key', 'liveness', 'loudness', 'mode', 'speechiness', 'tempo', 'time_signature', 'valence', 'track_name', 'artist_name', 'popularity']
...
```{r}
xx<-spotifyr::get_user_playlists(user_id = 'hannahluebbering')
data.frame(xx) %>% dplyr::select('id', 'name') %>%
  kable()
