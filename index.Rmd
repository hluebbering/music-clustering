...
```{r}
library(DT)
datatable(py$links_dict%>% data.frame() %>% head(10), rownames = FALSE, options = list(pageLength = 3, scrollX=T) )
...
datatable(py$means, rownames =  c("Cluster 1", "Cluster 2", "Cluster 3"), options = list(pageLength = 3, scrollX=T) )
...
Through machine learning and collaborative filtering related approaches, we can break down and analyze a playlist to gain insights into listening preferences and habits while also reccomending similar taste in music. For example, if a playlist contains multiple songs from a particular artist, the algorithm may recommend similar artists that other users who like that artist also listen to.

### Next Steps 

An extended approach to this project includes using **Natural Language Processing** to track music. Essentially, NLP searches for any mentions of a particular artist or song in order to analyze what people are talking about online, on social media or blogs articles. New technologies can identify top keywords and terms that are related to a song, and then make recommendations of other songs that have similar terms through NLP. Essentially NLP can categorize songs into groups and give each track descriptive adjectives to further classify each individual song. 
