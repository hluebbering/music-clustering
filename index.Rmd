...
x <- py$data_pca
lss = c()
for (i in py$kmeans$labels_) {
  lss = append(x = lss, values = glue::glue("Group ", i))}
pcadf <- data.frame("x1" = x[,1], "x2" = x[,2], "labels"=lss)

data_points_plot <- ggplot(data = pcadf, mapping = aes(x = x1, y = x2, label = labels)) +
  geom_point(mapping = aes(fill = labels), pch = 21, size = 3.25, alpha = 0.9, color = "#030404", stroke = 0.5) +
...
```{python}
df2 = df[['acousticness', 'danceability', 'energy', 'instrumentalness', 'liveness', 'speechiness', 'valence','group']]
df2 = df2.astype({'group': str})

means = pd.DataFrame(index = range(0,3), columns = list(df2[df2['group'] == '0'].describe().loc['mean'].index))
means.iloc[0] = df2[df2['group'] == '0'].describe().loc['mean']
means.iloc[1] = df2[df2['group'] == '1'].describe().loc['mean']
means.iloc[2] = df2[df2['group'] == '2'].describe().loc['mean']
```
